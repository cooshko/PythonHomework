...
import os, sys, re, socketserver, logging, hashlib, selectors, socket
...
class FtpServer(object):
    ...
    def __init__(self, request):
        ...
        self.client_address = request.getsockname()
    ...
    def serve():
        ...
        sock.listen(5)
        FtpServer.SEL = selectors.SelectSelector()
        FtpServer.SEL.register(sock, selectors.EVENT_READ, FtpServer.accept)
        while True:
            for key, mask in FtpServer.SEL.select():
                callback = key.data
    ...
    def accept(sock, mask):
        ...
        server = FtpServer(conn)
        FtpServer.SEL.register(conn, selectors.EVENT_READ, server.handle)
