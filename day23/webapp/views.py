...
def score(request):
    ...
    if request.method == 'GET':
        ...
        fail_list = []
        need_re_avg = False     # 判断是否需要重新计算课程的平均成绩，不管是修改成绩还是新增成绩都应重新计算平均成绩
        ...
        if need_re_avg:
            ...
            score_sum = sum([x['score'] for x in all_score])
            avg_score = int(score_sum/score_count)
